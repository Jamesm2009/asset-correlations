...
- 45 ETFs + 35 Mutual Funds, 21-day and 5-day rolling correlations
- Data: yFinance (91 trading days of daily closes)
- Cache: Upstash Redis — one key holds all prices, filtered per request
...
MF_TICKERS = sorted([
    "COSYX","DFREX","FBNRX","GCCUX","HBLVX","HFMVX","JLGMX","JSVUX",
    "MCZZX","MPHZX","NOBOX","PIMIX","PRRIX","RERGX","RLLGX","RNWGX",
    "RWMGX","TRRAX","TRRBX","TRRCX","TRRDX","TRRFX","TRRGX","TRRHX",
    "TRRJX","TRRKX","TRRLX","TRRMX","TRRNX","VEXRX","VFIAX","VIMAX",
    "VMVAX","VSMAX","VTIAX",
])

ALL_TICKERS = sorted(set(ETF_TICKERS + MF_TICKERS))
...
def fetch_prices():
    ...
    print(f"Downloading {len(ALL_TICKERS)} tickers (ETFs + MFs) {start} -> {end}...")
    with _lock:
...
def prices_for_universe(prices_dict, universe):
    """Filter the full prices dict to ETF or MF tickers only."""
    allowed = set(ETF_TICKERS if universe == "etf" else MF_TICKERS)
    return {k: v for k, v in prices_dict.items() if k in allowed}
...
def run_update():
    ...
    print(f"Update complete — {len(prices_dict)} tickers — {last_updated}")
...
def api_matrix():
    ...
    universe = request.args.get("universe", "etf", type=str).lower()
    if window not in (5, 21):
        window = 21
    if universe not in ("etf", "mf"):
        universe = "etf"
    ...
    filtered = prices_for_universe(prices_dict, universe)
    if not filtered:
        return jsonify({"error": "No data for this universe"}), 503

    prices_df = pd.DataFrame(filtered)
